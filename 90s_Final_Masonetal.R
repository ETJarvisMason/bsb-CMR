# Mason et al.
# Capture-mark-reencounter (CMR) model for modeling the 1990s CDFW CMR data
# 2022
...
# Define model for 1990s CDFW CMR data--------------------------------------------

sink("tvr_size90s.jags")
cat("
  model{
  ...
  # for the 1990s, we have estimates of total BSB harvest in 1989, 1993, 1994, 1996, 1997, 1998, & 1999 
...
# Load CDFW BSB CMR data (1989-1999)-----------------------------------------------------------------------------

# load encounter history and length data
load("ch90sAnn.Rdata")
load("ch90sLen.Rdata")

df <- dat.final # ch obs matrix with fish ID

y2 <- data.matrix(df[,1:11]) # get rid of ID
y2[y2 == 2] <- 5 # only 1 biologist recap
...
jags.90s <- jags(jags.data, inits=NULL, parameters, "tvr_size90s.jags", n.chains = nc,
...
save(jags.90s,file="90s_final.Rdata")